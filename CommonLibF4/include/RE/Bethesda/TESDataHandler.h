...
namespace RE
{
	...
	public:
		...
		{
			REL::Relocation<TESDataHandler**> singleton{ REL::ID(711558, 4796135) };
			return *singleton;
		}
		...
		[[nodiscard]] bool AddFormToDataHandler(TESForm* a_form)
		{
			...
			REL::Relocation<func_t> func{ REL::ID(350112, 2192271) };
		...
		[[nodiscard]] bool CheckModsLoaded(bool a_everModded)
		{
			...
			REL::Relocation<func_t> func{ REL::ID(1432894, 2192323) };
		...
		[[nodiscard]] ObjectRefHandle CreateReferenceAtLocation(NEW_REFR_DATA& a_data)
		{
			...
			REL::Relocation<func_t> func{ REL::ID(500304, 2192301) };
			return func(this, a_data);
		}

		[[nodiscard]] ObjectRefHandle CreateProjectileAtLocation(
			TESBoundObject* a_projectileBase,
			const NiPoint3& a_position,
			const NiPoint3& a_angles,
			TESObjectCELL* a_cell,
			TESWorldSpace* a_world) noexcept
		{
			using func_t = ObjectRefHandle(
				TESDataHandler*,
				TESBoundObject*,
				const NiPoint3&,
				const NiPoint3&,
				TESObjectCELL*,
				TESWorldSpace*);
			REL::Relocation<func_t> func{ REL::ID(69438, 2192305) };
			return func(this, a_projectileBase, a_position, a_angles, a_cell, a_world);
		}
		...
		[[nodiscard]] std::uint32_t LookupFormID(std::uint32_t a_rawFormID, std::string_view a_modName)
		{
			...
			return (static_cast<std::uint32_t>(file->compileIndex) << 24) |
			       (static_cast<std::uint32_t>(file->smallFileCompileIndex) << 12) |
			       a_rawFormID;
		}

		[[nodiscard]] TESForm* LookupForm(std::uint32_t a_rawFormID, std::string_view a_modName)
		{
			const auto formID = LookupFormID(a_rawFormID, a_modName);
			return formID != 0 ? TESForm::GetFormByID(formID) : nullptr;
		}

		template <class T>
		[[nodiscard]] T* LookupForm(std::uint32_t a_rawFormID, std::string_view a_modName)
		{
			auto* form = LookupForm(a_rawFormID, a_modName);
			return form && form->Is(T::FORM_ID) ? form->As<T>() : nullptr;
		}

		[[nodiscard]] const TESFile* LookupModByName(std::string_view a_modName)
		{
			for (auto* file : files) {
				if (file && a_modName.size() == std::strlen(file->filename) &&
		...
		[[nodiscard]] std::optional<std::uint8_t> GetModIndex(std::string_view a_modName)
		{
			const auto* mod = LookupModByName(a_modName);
			return mod ? std::make_optional(mod->compileIndex) : std::nullopt;
		}

		[[nodiscard]] const TESFile* LookupLoadedModByName(std::string_view a_modName)
		{
		...
		[[nodiscard]] const TESFile* LookupLoadedModByIndex(std::uint8_t a_index)
		{
			for (auto* file : compiledFileCollection.files) {
		...
		[[nodiscard]] std::optional<std::uint8_t> GetLoadedModIndex(std::string_view a_modName)
		{
			const auto* mod = LookupLoadedModByName(a_modName);
			return mod ? std::make_optional(mod->compileIndex) : std::nullopt;
		}

		[[nodiscard]] const TESFile* LookupLoadedLightModByName(std::string_view a_modName)
		{
			for (auto* file : compiledFileCollection.smallFiles) {
				if (file && a_modName.size() == std::strlen(file->filename) &&
					_strnicmp(file->filename, a_modName.data(), a_modName.size()) == 0) {
					return file;
				}
		...
		[[nodiscard]] const TESFile* LookupLoadedLightModByIndex(std::uint16_t a_index)
		{
			for (auto* file : compiledFileCollection.smallFiles) {
				if (file && file->smallFileCompileIndex == a_index) {
					return file;
				}
			}
			return nullptr;
		...
		[[nodiscard]] std::optional<std::uint16_t> GetLoadedLightModIndex(std::string_view a_modName)
		{
			const auto* mod = LookupLoadedLightModByName(a_modName);
			return mod ? std::make_optional(mod->smallFileCompileIndex) : std::nullopt;
		}

		[[nodiscard]] bool IsFormIDInUse(std::uint32_t a_formID)
		{
			using func_t = decltype(&TESDataHandler::IsFormIDInUse);
			REL::Relocation<func_t> func{ REL::ID(1448838, 2192351) };
			return func(this, a_formID);
		}

		[[nodiscard]] bool IsFormIDInuse(std::uint32_t a_formID)
		{
			return IsFormIDInUse(a_formID);
		}
